-- Drop conflicting admin policies
...
-- Drop replacement policies if this repair script is rerun.
-- Do not recreate an admin FOR ALL policy on profiles: it can make normal
-- profile reads/writes fail if it recursively checks the profiles table.
...
-- Use a SECURITY DEFINER helper to avoid recursive RLS checks on profiles.
